...
results_process <- function(
    ## original data
    data = dta,
    ## parameters for imputation
    dta_imps = dta_imps,
    if_imps = FALSE, # whether to conduct the imputation
    outome = "outcome",
    ...
    analysis_m = 1,
    analysis = "ITT",
    if_boot = TRUE, # whether to boot (FALSE--> main analysis)
  ...
  if (if_imps) {
    ...
    dta_imps <- imputation(
      ...
      imputed_not_include_vars = imputed_not_include_vars,
      cores_2_use = imputed_cores_2_use,
      m = imputed_m
    )
    message("imputation process save file")
    imps_filename <- paste0("dta_imps_", intervention, control, "_", outcome, "_2014to2021.R")
  ...
  results <- lapply(1:analysis_m, function(imputation_no) {
    ...
    result_m <- foreach(
      ...
      result_k <- bootstrap_estimation_nosub(
        data = data_tidy[[1]],
        analysis = analysis,
        if_boot = if_boot,
  ...
  if (if_boot) {
    ...
    results_filename <- paste0(paste(analysis, intervention, control, outcome, "res", sep = "_"), ".R")
